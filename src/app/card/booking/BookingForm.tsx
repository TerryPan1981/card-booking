...
function qualificationFields(mode: BookingMode, intent: string) {
  if (mode === "collaboration") {
    return [
      ...
      { key: "purpose", label: "希望合作的內容", placeholder: "請說明形式、對象與希望阿睿參與的部分", required: true, multiline: true },
  ...
  if (intent === "sell") {
    return [
      { key: "propertyAddress", label: "物件大約位置", placeholder: "例：板橋區文化路，填到路名即可", required: true },
...
export default function BookingForm() {
  ...
  const validate = (): FieldErrors => {
    ...
    if (meetType === "custom" && !customReady) next.meetType = "指定地點必須使用阿睿核准後提供的專屬連結。";
      ...
      <main className={styles.page}>
        <div className={styles.shell}>
          <header className={styles.header}>
            <div className={styles.brand}>房仲好鄰居－阿睿</div>
  ...
  return (
    <main className={styles.page}>
      <div className={styles.shell}>
        <header className={styles.header}>
          <div className={styles.brand}>房仲好鄰居－阿睿</div>
          <h1 className={styles.title}>預約與阿睿聊聊</h1>
          ...
          {bookingMode && intentKey ? (
            <section className={styles.section}>
              ...
              {approvalState === "approved" && !approvedLocation ? (
                <div className={styles.errorNotice}>核准連結缺少已同意的地點，無法開放指定地點。請聯絡阿睿重新產生連結。</div>
              ) : null}
              {approvalState === "invalid" ? (
                <div className={styles.errorNotice}>指定地點核准連結已失效、已使用或被撤銷，請重新聯絡阿睿。</div>
              ) : null}
              ...
              {!customReady && approvalState !== "checking" ? (
                <div className={styles.inlineNotice}>
                  需要指定其他地點？請先
                  {" "}<a className={styles.link} href={SOCIAL.line} target="_blank" rel="noopener noreferrer">加阿睿私人 LINE</a>
                  {" "}確認。取得專屬連結後，系統會直接帶入核准地點。
                </div>
              ) : null}
              {meetType === "custom" && approvedLocation ? (
                <div className={styles.field}>
                  <span className={styles.label}>阿睿已核准的指定地點</span>
                  ...
                  <p className={styles.sectionHint}>此地點已鎖定，若要更換請先與阿睿重新確認。</p>
                </div>
              ) : null}
            </section>
          ) : null}
              ...
              <div ref={registerRef("slot")} tabIndex={-1}>
                ...
                {!loadingSlots && !slotError && days.length === 0 ? (
                  <div className={styles.inlineNotice}>
                    ...
                    {" "}<a className={styles.link} href={SOCIAL.line} target="_blank" rel="noopener noreferrer">直接聯絡阿睿</a>。
                  </div>
                ) : null}
                {days.length ? (
                  <>
